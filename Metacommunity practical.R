...
# Simulate 10 species over 50 discrete sites
#define sites and their characteristics:
...
#plot the sites colouring them according to their environment:
plot(y ~ x, my.landscape$site.coords, pch=16, 
     col=tim.colors(n=64)[ceiling(my.landscape$site.info$Ef*64)])

#define 10 species, including their niches along the environmental axis
#each niche is modelled as a Normal distribution, typical for Hutchinsonian niches
...
head(sim.result$J.long) # main output of the function:: contains species composition of all sites over all the simulated timesteps

# extract the part of the data frame corresponding to the last time step, transform it to a species-by-site df:
...
#Run this code at the end of the previous one:
...
# Explore the influence of dispersal ability and niche width on the model

# Initialise vectors for the for loops
...
# these numbers are different parameter values that they will take on 

#store diversity values in a 5x6 matrices
Gamma <- Alpha <- Shannon <- beta.Sim <- beta.Sor <- beta.ab <- matrix(NA,length(niche.width),length(disp.ab))

#store MC.fin.mat in a list of list
MC.fin.mat.store <- list()
MC.fin.mat.store

# Add the two for() loops to your code, and the indices to the patterns and run the model
# CODE in Model_explore script